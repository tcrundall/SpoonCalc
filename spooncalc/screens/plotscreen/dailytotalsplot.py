...
from spooncalc.models.activitylog import QUALIFIERS, ActivityLog
...
class DailyTotalsPlot:
    ...
    def __init__(self, db: Database):
        ...
        self.data: Dict[YMode, Dict[str, Dict[int, float]]] = {
            YMode.SPOONS: {
                q: defaultdict(float)
                for q in QUALIFIERS + ["total", "averaged"]
            },
            YMode.HOURS: {
                q: defaultdict(float)
                for q in QUALIFIERS + ["total", "averaged"]
    ...
    def update_data(self) -> None:
        for day_offset in range(self.xmin, self.xmax + 1):
            ...
            if day_offset not in self.logs_by_day.keys():
                self.logs_by_day[day_offset] =\
                    self.db.get_logs_from_day(day_offset)

                # Collate data into points? Dicts?
                self.data[YMode.HOURS]["total"][day_offset] =\
                    sum([log.hours for log in self.logs_by_day[day_offset]])
                self.data[YMode.SPOONS]["total"][day_offset] =\
                    sum([log.spoons for log in self.logs_by_day[day_offset]])
                ...
                for log in self.logs_by_day[day_offset]:
                    ...
                    for qual in QUALIFIERS:
                        if getattr(log, qual):
                            for ymode in YMode:
                                self.data[ymode][qual][day_offset] +=\
                                    getattr(log, ymode.value)
